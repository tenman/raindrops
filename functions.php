...
?><?php
	...
	if ( !is_admin() ) {
		wp_register_script('raindrops_script',get_bloginfo('stylesheet_directory') .'/lib/script.php',array(),'0.1' );
	...
	if ( function_exists('register_sidebars') ){
		...
		register_sidebar(  
		  ...
		  'after_widget' => '</li>',
		  'before_title' => '<h2 class="widgettitle footer-content h2">',
		  'after_title' => '</h2>',
		  'widget_id' => 'toppage1',
		  'widget_name' => 'toppage1',
		  'text' => "4")
		  );
	}	  
...
function add_body_class($class) {
	...
	function raindrops_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
			...
			<div class="comment-meta commentmetadata" style="overflow:hidden;*width:100%;padding-left:1em;" class="clearfix"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
				<?php
...
echo "<h2>".__('Welcome Raindrops.').'</h2>';


echo "<p>".__('Your own blog offers some methods to improve it better to this theme file.').'</p>';
echo "<p>".__('For instance, it is a color.').'</p>';
echo "<p>".__('The color is influencing even the person who doesn\'t stick to decorated thing.').'</p>';
echo "<p>".__('Therefore, the color was made easy to change. ').'</p>';
echo "<p>".__('For instance, it is a layout. ').'</p>';
echo "<p>".__('There is a legible document even if the same thing was written, and is a document not read easily. ').'</p>';
echo "<p>".__('A legible document is one index in an excellent document. ').'</p>';
echo "<p>".__('Therefore, the layout was able to be tested.').'</p>';  
echo "<p>".sprintf(__('WEBSite：<a href="%1$s">%2$s</a>'),'http://www.tenman.info/raindrops','Raindrops').'</p>';
...
class tmn_menu_create {
	...
	global $raindrops_base_setting;
		...
		foreach($results as $key=>$result){	
			...
			$lines .= "</tbody>";
			$lines .= '<tfoot><tr><th>id</th><th>名称</th><th >値</th><th>設定</th></tr></tfoot>';
			$lines .= "</table></div>";
