...
/**
 * Raindrops 1.253 add new script loader
 * This variable as soon as can be confirmed that there are no problems will be removed
 * set false then old script load
 */
if ( ! isset( $raindrops_new_style_load ) ) {

	$raindrops_new_style_load = true;
}
if ( ! isset( $raindrops_new_script_load ) ) {
	$raindrops_new_script_load = true;
}
/**
...
if ( is_admin() && $raindrops_recommend_plugins !== false ) {
	...
	if ( file_exists( get_stylesheet_directory() . '/plugins/tgm-config.php' ) ) {

		require_once ( get_stylesheet_directory() . '/plugins/tgm-config.php' );
		$raindrops_is_include_tgm_config = true;
	} elseif ( file_exists( get_template_directory() . '/plugins/tgm-config.php' ) ) {

		require_once ( get_template_directory() . '/plugins/tgm-config.php' );
		$raindrops_is_include_tgm_config = true;
	}
...
if ( file_exists( get_stylesheet_directory() . '/plugins/plugins-presentation.php' ) ) {

	require_once ( get_stylesheet_directory() . '/plugins/plugins-presentation.php' );
} else {

	require_once ( get_template_directory() . '/plugins/plugins-presentation.php' );
}
/**
 *
 *
...
LINK_COLOR_CSS;
	...
	if ( !function_exists( "raindrops_embed_meta" ) ) {
		...
		function raindrops_embed_meta( $content ) {
			...
			global $post, $wp_customize, $raindrops_new_style_load;
			$result	 = "";
//I will be removed in version 1.255
			if( $raindrops_new_style_load == true ) {
				$css     = '';
			} else {
			...
			if ( isset( $wp_customize ) || $raindrops_new_style_load == false ) {
	...
	if ( !function_exists( "raindrops_loop_title" ) ) {
		...
		function raindrops_loop_title() {
			...
			if ( !empty( $Raindrops_class_name ) ) {
				...
				echo "\n". str_repeat("\t", 7 ). '<ul class="index ' . esc_attr( $Raindrops_class_name ) . '">';
			} else {

				echo "\n". str_repeat("\t", 7 ). '<ul class="index">';
			}
...
///////////////////////////////////test
	...
	if ( !function_exists( "raindrops_color_type_custom" ) ) {
		function raindrops_color_type_custom( $css ) {
			...
			}elseif( intval( get_query_var( 'raindrops_color_type' ) ) == 1 && $post_id = get_query_var( 'p' ) && $raindrops_new_style_load == true )  {
	 ...
	 *
	 */
//I will be removed in version 1.255  conditional if
			if ( true == $raindrops_new_script_load ) {
	 ...
	 *
	 */
//I will be removed in version 1.255 raindrops_helper_for_no_fluid_page_width
...
//I will be removed in version 1.255 raindrops_small_device_helper
		...
		function raindrops_entry_title( $args = array() ) {
			...
			*/
			...
			if ( !is_singular() || is_page_template( 'page-templates/list-of-post.php' ) || ( is_page_template( 'page-templates/full-width.php' ) && is_front_page()  ) || ( is_page_template( 'page-templates/front-page.php' ) && is_front_page()  ) ) {
				...
				$html = sprintf( $html, 'h2 entry-title', the_title( '', '', false ), $raindrops_unique_label, $thumbnail );
 ...
 * @since 1.254
 */
global $wp_customize,$raindrops_new_style_load;




if ( ! isset( $wp_customize ) && $raindrops_new_style_load == true ) {
	/**
	 * @1.254
	 */
...
if ( !function_exists( 'raindrops_register_color_type_style' ) ) {
    ...
    function raindrops_register_color_type_style() {
		...
		global $raindrops_current_data_version,$post,$posts;
		$query = '';
		$count = count( $posts );
		if( isset($post) && $count == 1 ) {

			$query = sprintf( '&amp;p=%1$s', $post->ID);
		}
			$version     = md5( raindrops_embed_css() );

            wp_register_style( 'raindrops_color_type', sprintf('/?%1$s=1%2$s', 'raindrops_color_type', $query ),array(), $version );
...
if ( !function_exists( 'raindrops_color_type_style_buffer' ) ) {
    ...
    function raindrops_color_type_style_buffer( ) {
		global $raindrops_fallback_human_interface_show,$post;
...
if ( !function_exists( 'raindrops_footer_text' ) ) {
	...
	function raindrops_footer_text() {
		...
		$raindrops_address_rss = '<small>&copy;%1$s &nbsp; %2$s &nbsp;<a href="%3$s" class="entry-rss">%4$s</a> <span>' . esc_html__( 'and', 'Raindrops' ) . '</span><a href="%5$s" class="comments-rss">%6$s</a>';
		...
		$raindrops_address_html .= sprintf( '&nbsp;<small><a href="%s">%s</a></small>&nbsp;&nbsp;', $raindrops_current_data_theme_uri, $raindrops_theme_name
		);
