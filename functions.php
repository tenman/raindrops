	...
	function raindrops_comment( $comment, $args, $depth ) {
		...
		if ( 'comment' == $comment->comment_type ) {
			...
			echo esc_url( get_comment_link( $comment->comment_ID ) );
						?>"><?php printf( esc_html__( '%1$s at %2$s', 'raindrops' ), get_comment_date(), get_comment_time() ); ?></a>
							<?php
		...
		if ( ! function_exists( 'raindrops_settings_page_contextual_help' ) ) {
			...
			function raindrops_settings_page_contextual_help() {
				...
				/* Changelog.txt */
				$content .= sprintf( $html, esc_html__( 'Change log text', 'raindrops' ), sprintf( $link, esc_url( get_template_directory_uri() . '/changelog.txt' ), esc_html__( 'Changelog , display new window', 'raindrops' ), 'target="_blank"' ), 'target = "_blank"' );
				/* readme.txt */
				$content .= sprintf( $html, esc_html__( 'Readme text', 'raindrops' ), sprintf( $link, esc_url( get_template_directory_uri() . '/README.txt' ), esc_html__( 'Readme , display new window', 'raindrops' ), 'target="_blank"' ) );
				...
				if ( file_exists( get_stylesheet_directory() . '/images/' . $filename ) ) {
					...
					if ( '#hd' == $id ) {
						...
						if ( ! file_exists( $upload_info['path'] . '/' . $filename ) ) {
							$url = get_stylesheet_directory_uri() . '/images/' . $filename;
							return apply_filters( 'raindrops_upload_image_parser_hd', 'background:url(  ' . esc_url( $url ) . '  );background-repeat:repeat-x;' );
						}
					} elseif ( '#ft' == $id ) {

						if ( ! file_exists( $upload_info['path'] . '/' . $filename ) ) {
							$url = get_stylesheet_directory_uri() . '/images/' . $filename;
							return apply_filters( 'raindrops_upload_image_parser_ft', 'background:url(  ' . esc_url( $url ) . '  );background-repeat:repeat-x;' );
						}
					}
					...
					if ( '#hd' == $id ) {
						...
						if ( ! file_exists( $upload_info['path'] . '/' . $filename ) ) {
							$url = get_template_directory_uri() . '/images/' . $filename;
							return apply_filters( 'raindrops_upload_image_parser_hd', 'background:url(  ' . esc_url( $url ) . '  );background-repeat:repeat-x;' );
						}
					} elseif ( '#ft' == $id ) {
		...
		if ( ! function_exists( "raindrops_add_stylesheet" ) ) {
			...
			function raindrops_add_stylesheet() {
				...
				if ( true == $raindrops_fallback_human_interface_show ) {
					...
					$fallback_style = esc_url( get_template_directory_uri() . '/fallback.css' );
				...
				if ( $raindrops_css_auto_include == true ) {
					...
					$style_filename = esc_url( get_template_directory_uri() . '/style.css' );

					if ( file_exists( get_template_directory() . '/style' . $raindrops_minified_suffix . '.css' ) && true == $raindrops_load_minified_css_js ) {
						$style_filename = esc_url( get_template_directory_uri() . '/style' . $raindrops_minified_suffix . '.css' );
					}
				...
				if ( is_child_theme() ) {
					...
					$style_filename = esc_url( get_stylesheet_directory_uri() . '/style.css' );

					if ( file_exists( get_stylesheet_directory() . '/style' . $raindrops_minified_suffix . '.css' ) && true == $raindrops_load_minified_css_js ) {
						$style_filename = esc_url( get_stylesheet_directory_uri() . '/style' . $raindrops_minified_suffix . '.css' );
					}
...
LINK_COLOR_CSS;
		...
		if ( ! function_exists( "raindrops_get_day" ) ) {
			...
			function raindrops_get_day( $posts = '', $year = '', $mon = '', $day = '',
					$pad = 1 ) {
				global $month;
				...
				for ( $i = 0; $i <= 24; $i ++ ) {
					...
					if ( isset( $today[$i] ) ) {
						...
						foreach ( $today[$i] as $mytime ) {
							...
							$output .= sprintf( $title_html, esc_url( get_permalink( $mytime->ID ) ), absint( $mytime->ID ), $post_title
							);
						}
					} else {
		...
		if ( ! function_exists( "raindrops_month_list" ) ) {
			...
			<span class="author vcard"><a class="url fn nickname" href="%10$s" title="%11$s">%12$s</a></span> 					</li>';
								...
								$links			 .= sprintf( $html, 'h2 entry-title', esc_url( get_permalink( $month->ID ) ), 'link to content: ' . esc_attr( strip_tags( $month->post_title ) ), $month->post_title, $month->ID, ' ' . raindrops_post_class( array( 'clearfix' ), $month->ID, false ), raindrops_doctype_elements( 'span', 'time', false ), raindrops_doctype_elements( '', 'datetime="' . esc_attr( get_the_date( 'c' ) ) . '"', false ), $month->post_date, get_author_posts_url( get_the_author_meta( 'ID' ) ),
										/* translators: 1: post author display name */ sprintf( esc_attr__( 'View all posts by %s', 'raindrops' ), $display_name ), $display_name
								);
								$c ++;
							}
						}
		...
		if ( ! function_exists( "raindrops_delete_post_link" ) ) {
			 ...
			 * @return type
			 */
			function raindrops_delete_post_link( $link_text = null, $before = '',
					$after = '', $id = 0, $echo = true ) {
	...
	\n#" . $pw . '{margin:auto;text-align:left;' . "\n" . 'width:' . $width . 'em;' . $add_ie . $px . '}' . "\n#container{width:" . $raindrops_main_width . 'em;}/* raindrops is fixed end */';
		...
		if ( ! function_exists( 'raindrops_entry_title' ) ) {
			...
			function raindrops_entry_title( $args = array(), $post_id = 0 ) {
				...
				if ( $post_id !== 0 && is_int( $post_id ) ) {
					...
					$html	 = sprintf( $html, apply_filters( 'raindrops_entry_title_class', 'h2 entry-title' ), $title, $raindrops_unique_label, raindrops_post_thumbnail_for_loop_title( $post_id, $title )
					);
					return $html;
				}
				...
				if ( ! is_singular() || is_page_template( 'page-templates/list-of-post.php' ) || ( is_page_template( 'page-templates/full-width.php' ) && is_front_page() ) || ( is_page_template( 'page-templates/front-page.php' ) && is_front_page() ) ) {
					...
					$html = sprintf( $html, apply_filters( 'raindrops_entry_title_class', 'h2 entry-title' ), //1
							$thumbnail, //2
							get_permalink(), //3
		...
		if ( ! function_exists( 'raindrops_post_thumbnail_for_loop_title' ) ) {
			 ...
			 * @return type
			 */
			function raindrops_post_thumbnail_for_loop_title( $post_id = 0, $title = '',
					$in_the_loop = true ) {
				...
				if ( ! is_admin() ) {
					...
					if ( false === $format ) {
						...
						$image_uri		 = esc_url( get_template_directory_uri() . '/images/link.png' );
						$class			 = 'icon-link-no-title entry-title-text';
						$format_label	 = esc_html__( 'Article', 'raindrops' );
					} else {

						$image_uri	 = esc_url( get_template_directory_uri() . '/images/post-format-' . $format . '.png' );
		...
		if ( ! function_exists( 'raindrops_html_excerpt_with_elements' ) ) {
			 ...
			 * @since 1.278
			 */
			function raindrops_html_excerpt_with_elements( $content, $length = 200,
					$more = '...', $exact = true, $allow_html = false ) {
				global $post;
		...
		if ( ! function_exists( 'raindrops_entry_content' ) ) {
			 ...
			 * @since 0.980
			 */
			function raindrops_entry_content( $more_link_text = null,
					$stripteaser = false ) {
...
%7$s%8$s</div><h3 class="entry-title"><a href="%1$s" class="trancate" data-rows="' . absint( $settings['raindrops_show_related_posts_line_clip'] ) . '">%2$s</a></h3><div class="entry-content clearfix">%9$s</div></%4$s></li>';
				...
				if ( $posts ) {
					...
					foreach ( $posts as $post ) {
						...
						$classes = 'class="' . join( ' ', $classes ) . '"';
						$result	 .= sprintf( $html, esc_url( get_permalink( $post->ID ) ), get_the_title(), $list_num_class, raindrops_doctype_elements( 'div', 'article', false ), $post->ID, $classes, sprintf( '<a href="%1$s" title="%2$s"><%4$s class="entry-date" %5$s>%3$s</%4$s></a>&nbsp;', $day_link, esc_attr( 'archives daily ' . mysql2date( get_the_date(), $raindrops_date_format ) ), esc_html( $date_strings ), raindrops_doctype_elements( 'span', 'time', false ), raindrops_doctype_elements( '', 'datetime="' . esc_attr( get_the_date( 'c' ) ) . '"', false )
										...
										/* translators: 1: author name */ sprintf( esc_attr__( 'View all posts by %s', 'raindrops' ), get_the_author() ), get_the_author()
								), wp_html_excerpt( $post_content, $raindrops_excerpt_length, $raindrops_excerpt_more ) . $oembed_flag, $thumbnail, $article_margin
						);
					}
					$result .= sprintf( '</ul>' );
...
%7$s%8$s</div><h3 class="entry-title"><a href="%1$s" class="trancate" data-rows="' . absint( $settings['raindrops_show_related_posts_line_clip'] ) . '">%2$s</a></h3><div class="entry-content clearfix">%9$s</div></%4$s></li>';
		...
		if ( ! function_exists( 'raindrops_monthly_archive_prev_next_navigation' ) ) {
			 ...
			 * @return type
			 */
			function raindrops_monthly_archive_prev_next_navigation( $echo = true,
					$show_year = false ) {
...
LIMIT 1" );
					...
					if ( $previous ) {
						...
						$calendar_output = sprintf( $html, get_month_link( $previous->year, $previous->month ),
								/* translators: 1: previous month name */ sprintf( esc_html__( '&#171; %s ', 'raindrops' ), $previous_label ), 'alignleft' );
					}
					$calendar_output .= "\t";
					...
					if ( $next ) {
						...
						$calendar_output .= sprintf( $html, get_month_link( $next->year, $next->month ),
								/* translators: 1: next month name */ sprintf( esc_html__( ' %s &#187;', 'raindrops' ), $next_label ), 'alignright' );
					}
		...
		if ( ! function_exists( 'raindrops_customize_controls_print_styles' ) ) {
			...
			<style <?php raindrops_doctype_elements( ' type="text/css" ', '' ); ?> id="raindrops-customizer-1">
				...
				#customize-control-raindrops_style_type .customize-control-title + label{
					...
					background:url( <?php echo esc_url( get_template_directory_uri() . '/images/screen-shot-dark.png' ); ?> );
					height:200px;
					display:block;
				...
				#customize-control-raindrops_style_type .customize-control-title  + label + label{
					...
					background:url( <?php echo esc_url( get_template_directory_uri() . '/images/screen-shot-w3standard.png' ); ?> );
					height:200px;
					display:block;
					background-position:0px 40px;
					background-repeat:no-repeat;
				...
				#customize-control-raindrops_style_type .customize-control-title  + label +label + label{
					...
					background:url( <?php echo esc_url( get_template_directory_uri() . '/images/screen-shot-light.png' ); ?> );
					height:200px;
					display:block;
					background-position:0px 40px;
					background-repeat:no-repeat;
				...
				#customize-control-raindrops_style_type .customize-control-title  + label +label + label + label{
					...
					background:url( <?php echo esc_url( get_template_directory_uri() . '/images/screen-shot-minimal.png' ); ?> );
					height:200px;
					display:block;
					background-position:0px 40px;
					background-repeat:no-repeat;
		...
		if ( is_child_theme() ) {
			?>
					...
					#customize-control-raindrops_style_type .customize-control-title  + label +label + label + label + label{
						...
						*/
						background:url( <?php echo esc_url( get_stylesheet_directory_uri() . '/screenshot.png' ); ?> ),
							url( <?php echo esc_url( get_template_directory_uri() . '/images/screen-shot-child-fallback.png' ); ?> );
						height:200px;
						display:block;
						background-position:0px 40px;
						background-repeat:no-repeat;
...
if ( ! function_exists( 'raindrops_loop_class' ) ) {
	 ...
	 * @since 1.001
	 */
	function raindrops_loop_class( $raindrops_loop_number,
			$raindrops_tile_post_id = '', $add_class = '' ) {
...
if ( ! function_exists( 'raindrops_nav_menu_primary' ) ) {
	...
	function raindrops_nav_menu_primary( $args = array() ) {
		...
		if ( "show" == raindrops_warehouse( 'raindrops_show_menu_primary' ) ) {
			...
			$html = sprintf( $template, esc_attr( raindrops_warehouse( 'raindrops_page_width' ) ), $raindrops_nav_menu_primary, raindrops_doctype_elements( 'div', 'nav', false ), raindrops_doctype_elements( '', 'aria-label="' . esc_attr__( 'Primary Navigation', 'raindrops' ) . '"', false ), esc_attr__( 'Primary Navigation', 'raindrops' ) );
...
if ( ! function_exists( 'raindrops_link_unique' ) ) {
	 ...
	 * @since 1.116
	 */
	function raindrops_link_unique( $text = '', $id = 0,
			$class = 'raindrops_unique_identifier' ) {
...
if ( ! function_exists( 'raindrops_poster' ) ) {
	...
	function raindrops_poster( $args ) {
		...
		if ( ! function_exists( 'raindrops_comment_class' ) ) {
			 ...
			 * @since 1.136
			 */
			function raindrops_comment_class( $comment_class = array(),
					$add_start_attribute = true ) {
		...
		if ( ! function_exists( 'raindrops_apply_google_font_import_rule_for_article_title' ) ) {
			...
			function raindrops_apply_google_font_import_rule_for_article_title( $css ) {
				...
				if ( ! empty( $setting_value ) ) {
					...
					if ( ! is_array( $fonts_get ) ) {

						return $css;
					}
					$import_rule = ! empty( $fonts_get['import_rule'] ) ? $fonts_get['import_rule'] : '';
		...
		if ( ! function_exists( 'raindrops_apply_google_font_styles_for_article_title' ) ) {
			...
			function raindrops_apply_google_font_styles_for_article_title( $css ) {
				...
				if ( ! empty( $setting_value ) ) {
					...
					$style		 = ! empty( $fonts_get['apply_style'] ) ? $fonts_get['apply_style'] : '';
...
POST_FORM;
		...
		if ( ! function_exists( 'raindrops_get_the_posted_in_category' ) ) {
			 ...
			 * @since 1.337
			 */
			function raindrops_get_the_posted_in_category( $separator = '',
					$parents = '', $post_id = false ) {
		...
		if ( ! function_exists( 'raindrops_get_the_posted_in_tag' ) ) {
			 ...
			 * @since 1.337
			 */
			function raindrops_get_the_posted_in_tag( $before = '', $sep = '',
					$after = '' ) {
		...
		if ( ! function_exists( 'raindrops_gettext_with_context' ) ) {
			 ...
			 * @since 1.411
			 */
			function raindrops_gettext_with_context( $translated, $text, $context,
					$domain ) {
		...
		if ( ! function_exists( 'raindrops_admin_post_stylesheet' ) ) {
			...
			function raindrops_admin_post_stylesheet() {
				...
				wp_enqueue_style( 'admin_options', esc_url( get_template_directory_uri() . '/admin-options.css' ) );
			}
		...
		if ( ! function_exists( 'raindrops_filter_custom_post_archive_widget_title' ) ) {
			 ...
			 * @1.440
			 */
			function raindrops_filter_custom_post_archive_widget_title( $title,
					$instance, $id_base ) {
		...
		if ( ! function_exists( 'raindrops_filter_custom_post_category_widget_title' ) ) {
			 ...
			 * @1.440
			 */
			function raindrops_filter_custom_post_category_widget_title( $title,
					$instance, $id_base ) {
...
GRID_POSTED_IN;
	...
	.entry-content h3{
		...
		if ( ! function_exists( 'raindrops_archive_link_format' ) ) {
			 ...
			 * @return type
			 */
			function raindrops_archive_link_format( $link_html, $url, $text, $format,
					$before, $after ) {
...
<span class="rd-archive-count">%3$s</span></a></li>';
		...
		if ( ! function_exists( 'raindrops_add_codemirror_for_raindrops_custom_css_field' ) ) {
			...
			function raindrops_add_codemirror_for_raindrops_custom_css_field() {
				...
				wp_add_inline_script(
						'code-editor', sprintf(
								'jQuery( function() { wp.codeEditor.initialize( "raindrops_custom_css_field", %s ); } );', wp_json_encode( $settings )
						)
				);
			}
	...
	.paragraph_wrap_enable .size4of5{margin-left:auto;margin-right:auto;}';
		...
		if ( ! function_exists( 'raindrops_convert_inline_style_to_attribute_style' ) ) {
			 ...
			 * @since 1.511
			 */
			function raindrops_convert_inline_style_to_attribute_style( $tag, $handle,
					$src ) {
				global $post;
		...
		if ( ! function_exists( 'raindrops_search_from_terms' ) ) {
			...
			function raindrops_search_from_terms( $target_terms = array( 'category', 'post_tag' ),
					$echo = true ) {
		...
		if ( ! function_exists( 'raindrops_add_lazyload_script' ) ) {
			...
			function raindrops_add_lazyload_script() {
				...
				$list							 = 'registered';
				$image_loader_url				 = esc_url( get_template_directory_uri() . '/images/loader.svg' );
			   ...
			   jQuery(this).attr('src', '" . $image_loader_url . "');
