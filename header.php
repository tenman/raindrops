...
$amp_class = ! empty( $amp_options ) ? sanitize_html_class( 'raindrops-amp-'.$amp_options['theme_support'] ): '';

$root_class = function_exists('is_amp_endpoint') && is_amp_endpoint() ? sprintf('class="%1$s"', $amp_class ): '';

?><!DOCTYPE html>
<html <?php language_attributes(); ?> <?php echo $root_class; ?>>
    ...
    <body id="<?php echo $raindrops_current_theme_slug; ?>" <?php body_class();?>><?php has_action( 'wp_body_open' ) ? do_action('wp_body_open') : ''; ?>
		...
		<?php raindrops_prepend_doc(); ?><header id="top">
			...
			if (! function_exists('is_amp_endpoint') || ! is_amp_endpoint() ) {
				if ( function_exists( 'has_header_video' ) && function_exists( 'the_custom_header_markup' ) && has_header_video() && ( true == is_home() && true == is_front_page() || false == is_home() && true == is_front_page() ) ) {
