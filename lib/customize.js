    ...
    */}).toString().match(/[^]*\/\*([^]*)\*\/\}$/)[1];
    ...
    $( document ).on( 'change', function(event) {
       ...
       if ( wp.customize.instance( 'raindrops_theme_settings[raindrops_reset_options]' ).get() == 'yes' && 'option' == raindrops_customizer_script_vars.setting_field_type ) {
        ...
        if ( raindrops_alert_flag == false && wp.customize.instance( 'raindrops_theme_settings[raindrops_style_type]' ).get() == 'minimal' 
            && raindrops_customizer_script_vars.raindrops_current_style_type !== 'minimal'
            && 'option' == raindrops_customizer_script_vars.setting_field_type ) {
                
                var message = confirm( raindrops_customizer_script_vars.color_type_minimal_confirm );
                ...
                if( message ) {               
                    ...
                    $("#customize-header-actions #save").click( function(){
                     ...
                     wp.customize.instance( 'raindrops_theme_settings[raindrops_style_type]' ).set( raindrops_customizer_script_vars.raindrops_current_style_type );
                }  
        }
    } );
...
( function ( $ ) {
    ...
    /*  @1.425 Todo
            wp.customize( 'raindrops_reset_options', function ( value ) {

                value.bind( function ( newval ) {
                    var raindrops_alert_flag = false;
                    if ( 'yes' == newval ) {
                        var confirmation = window.confirm( raindrops_customizer_script_vars.reset_confirm );
            ...
            wp.customize( 'raindrops_style_type', function ( value ) {
                ...
                value.bind( function ( newval ) {
                    var raindrops_alert_flag = false;
                    if ( 'minimal' == newval && raindrops_alert_flag == false && raindrops_customizer_script_vars.raindrops_current_style_type !== 'minimal' ) {
                        var confirmation = window.confirm( raindrops_customizer_script_vars.color_type_minimal_confirm );
                        if ( true == confirmation ) {
                           ...
                           $("#customize-header-actions #save").click( function(){
                            ...
                            wp.customize.instance( 'raindrops_style_type' ).set( raindrops_customizer_script_vars.raindrops_current_style_type );
                        }
                        var raindrops_alert_flag = true;
                    }
                } );
